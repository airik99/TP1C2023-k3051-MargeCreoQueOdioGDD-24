...
AS
  BEGIN
    ...
    INSERT INTO horarioxdia(ID_DIA, ID_HORARIO_LOCAL)
		SELECT DISTINCT (SELECT TOP 1 ID_DIA FROM MargeCreoQueOdioGDD.dia WHERE dia.NOMBRE = HORARIO_LOCAL_DIA) AS ID_DIA,
			            (SELECT TOP 1 ID_HORARIO_LOCAL FROM MargeCreoQueOdioGDD.horario_local WHERE horario_local.HORA_APERTURA = HORARIO_LOCAL_HORA_APERTURA
						AND horario_local.HORA_CIERRE = HORARIO_LOCAL_HORA_CIERRE) AS ID_HORARIO_LOCAL
		FROM gd_esquema.Maestra
...
CREATE PROCEDURE MargeCreoQueOdioGDD.migrar_cupones_descuento
 AS
  BEGIN
		...
		FROM gd_esquema.Maestra
        WHERE CUPON_NRO IS NOT NULL AND CUPON_NRO NOT IN (SELECT CODIGO FROM MargeCreoQueOdioGDD.cupon_descuento)
		UNION
		...
		FROM gd_esquema.Maestra
		WHERE CUPON_RECLAMO_NRO IS NOT NULL AND CUPON_RECLAMO_NRO NOT IN (SELECT CODIGO FROM MargeCreoQueOdioGDD.cupon_descuento)
	END
GO
  ...
  BEGIN
    ...
    INSERT INTO productoxpedido(ID_PEDIDO, ID_PRODUCTO, CANTIDAD, PRECIO_UNITARIO)
		SELECT (SELECT TOP 1 pedido.NRO_PEDIDO FROM MargeCreoQueOdioGDD.pedido WHERE pedido.NRO_PEDIDO = PEDIDO_NRO) AS ID_PEDIDO,
			   (SELECT ID_PRODUCTO FROM MargeCreoQueOdioGDD.producto WHERE producto.CODIGO_PRODUCTO = CODIGO_PRODUCTO) AS ID_PRODUCTO,
  ...
  BEGIN
    ...
    INSERT INTO reclamo(NRO_RECLAMO, ID_USUARIO, ID_PEDIDO, ID_OPERADOR, ID_ESTADO, DESCRIPCION, SOLUCION, ID_TIPO_RECLAMO, CALIFICACION, FECHA_HORA_INICIO, FECHA_HORA_SOLUCION)
		SELECT DISTINCT RECLAMO_NRO AS NRO_RECLAMO,
			   (SELECT TOP 1 ID_USUARIO FROM MargeCreoQueOdioGDD.usuario WHERE usuario.DNI = USUARIO_DNI) AS ID_USUARIO,
			   (SELECT TOP 1 NRO_PEDIDO FROM MargeCreoQueOdioGDD.pedido WHERE pedido.NRO_PEDIDO = NRO_PEDIDO) AS ID_PEDIDO, -- TODO: HAY PEDIDOS CON NRO REPETIDO Y SON DISTINTOS
...
AS
  BEGIN
    ...
    INSERT INTO envio_mensajeria(ID_USUARIO, ID_DIRECCION_ORIGEN, ID_MEDIO_PAGO, ID_TIPO_PAQUETE, ID_ENVIO, ID_ESTADO, FECHA_HORA_PEDIDO,
	FECHA_HORA_ENTREGA, DISTANCIA_KMS, CALIFICACION, OBSERVACIONES, PRECIO_POR_SEGURO, TOTAL_SERVICIO_MENSAJERIA)

		SELECT DISTINCT (SELECT TOP 1 ID_USUARIO FROM MargeCreoQueOdioGDD.usuario WHERE usuario.NOMBRE = USUARIO_NOMBRE AND usuario.APELLIDO = USUARIO_APELLIDO AND usuario.EMAIL = USUARIO_MAIL AND usuario.DNI = USUARIO_DNI) AS ID_USUARIO,
			   ...
			   (SELECT TOP 1 ID_MEDIO_PAGO FROM MargeCreoQueOdioGDD.medio_de_pago WHERE medio_de_pago.NUMERO_TARJETA = NUMERO_TARJETA AND medio_de_pago.MARCA = MARCA_TARJETA) AS ID_MEDIO_DE_PAGO,
			   (SELECT TOP 1 tipo_paquete.ID_TIPO_PAQUETE FROM MargeCreoQueOdioGDD.tipo_paquete WHERE tipo_paquete.TIPO_PAQUETE = PAQUETE_TIPO
			   AND tipo_paquete.VALOR_ASEGURADO  = PAQUETE_TIPO_PRECIO) AS ID_TIPO_PAQUETE,
			   (SELECT TOP 1 NRO_ENVIO FROM MargeCreoQueOdioGDD.envio WHERE envio.NRO_ENVIO = NRO_ENVIO) AS ID_ENVIO,
			   (SELECT estado_mensajeria.NOMBRE FROM MargeCreoQueOdioGDD.estado_mensajeria WHERE estado_mensajeria.NOMBRE = ENVIO_MENSAJERIA_ESTADO) AS ID_ESTADO,
...
--select * from MargeCreoQueOdioGDD.envio_mensajeria
