...
CODIGO_PRODUCTO NVARCHAR(255), --PK
ID_LOCAL INT NOT NULL, --FK
NOMBRE NVARCHAR(255),
DESCRIPCION NVARCHAR(255),
PRECIO FLOAT,
  ...
  BEGIN
    ...
    INSERT INTO usuario (NOMBRE, APELLIDO, DNI, FECHA_REGISTRO, TELEFONO, EMAIL, FECHA_NACIMIENTO, ID_DIRECCION)
		SELECT DISTINCT USUARIO_NOMBRE AS NOMBRE,
			   ...
			   from gd_esquema.Maestra 
			   INNER JOIN MargeCreoQueOdioGDD.direccion AS dir ON dir.CALLE_Y_NUMERO = DIRECCION_USUARIO_DIRECCION AND dir.NOMBRE_DIRECCION = DIRECCION_USUARIO_NOMBRE
  ...
  BEGIN
    ...
    INSERT INTO repartidor (NOMBRE, APELLIDO, DNI, TELEFONO, EMAIL, FECHA_NACIMIENTO, ID_TIPO_MOVILIDAD, ID_DIRECCION, ID_LOCALIDAD)
		SELECT DISTINCT REPARTIDOR_NOMBRE AS NOMBRE,
			   ...
			   mo.ID_TIPO AS ID_TIPO_MOVILIDAD,
			   ID_DIRECCION AS ID_DIRECCION,
			   lo.ID_LOCALIDAD AS ID_LOCALIDAD
			   FROM gd_esquema.Maestra
			   INNER JOIN MargeCreoQueOdioGDD.tipo_movilidad AS mo ON mo.TIPO_MOVILIDAD = REPARTIDOR_TIPO_MOVILIDAD
			   INNER JOIN MargeCreoQueOdioGDD.direccion AS dir ON dir.CALLE_Y_NUMERO = REPARTIDOR_DIRECION
 ...
 AS
  BEGIN
    ...
    INSERT INTO operador (NOMBRE, APELLIDO, DNI, FECHA_NACIMIENTO, EMAIL, ID_DIRECCION, TELEFONO)
		...
		FROM gd_esquema.Maestra
			   INNER JOIN MargeCreoQueOdioGDD.direccion AS dir ON dir.CALLE_Y_NUMERO = OPERADOR_RECLAMO_DIRECCION
  ...
  BEGIN
    ...
    INSERT INTO local(NOMBRE, ID_CATEGORIA, DESCRIPCION, ID_DIRECCION)
		SELECT DISTINCT LOCAL_NOMBRE AS NOMBRE,
			   ...
			   ID_DIRECCION AS ID_DIRECCION
		FROM gd_esquema.Maestra
		       INNER JOIN MargeCreoQueOdioGDD.direccion AS dir ON dir.CALLE_Y_NUMERO = LOCAL_DIRECCION
  ...
  BEGIN
    ...
    INSERT INTO medio_de_pago(NUMERO_TARJETA, MARCA, ID_TIPO_MEDIO_PAGO, ID_USUARIO)
		SELECT DISTINCT MEDIO_PAGO_NRO_TARJETA AS NUMERO_TARJETA,
			  ...
			  ID_TIPO AS ID_TIPO_MEDIO_PAGO,
			  ID_USUARIO AS ID_USUARIO
		FROM gd_esquema.Maestra
		INNER JOIN MargeCreoQueOdioGDD.tipo_medio_pago AS ti ON ti.TIPO_PAGO = MEDIO_PAGO_TIPO
		INNER JOIN MargeCreoQueOdioGDD.usuario AS us ON us.EMAIL = USUARIO_MAIL AND us.DNI = USUARIO_DNI
	    WHERE MEDIO_PAGO_TIPO IS NOT NULL
  END
GO
---------------------------- Horario Local ----------------------------
...
AS
  BEGIN
    ...
    INSERT INTO horario_local(HORA_APERTURA, HORA_CIERRE, ID_LOCAL)
		...
		FROM gd_esquema.Maestra
		INNER JOIN MargeCreoQueOdioGDD.local AS lo ON lo.NOMBRE = LOCAL_NOMBRE
